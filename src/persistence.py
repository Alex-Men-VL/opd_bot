...
class RedisPersistence(BasePersistence[UD, CD, BD]):
    ...
    async def _perform_initialization(self) -> None:
        ...
        if data_bytes is None:
            ...
            self.user_data = defaultdict(dict, db_data.get(self.user_data_key, {}))
            self.user_data.update(initial_data.get('user_data', {}))

            self.chat_data = defaultdict(dict, db_data.get(self.chat_data_key, {}))
    ...
    async def _load_redis(self) -> None:
        ...
        try:
            ...
            self.user_data = defaultdict(dict, data.get(self.user_data_key, {}))
            self.chat_data = defaultdict(dict, data.get(self.chat_data_key, {}))
    ...
    async def flush(self) -> None:
        ...
        if self.user_data or self.chat_data or self.bot_data or self.callback_data or self.conversations:
            await self._dump_redis()
